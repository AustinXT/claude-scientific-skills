---
name: cirq
description: 用于构建、模拟、优化和执行量子电路的量子计算框架。在处理量子算法、量子电路设计、量子模拟（无噪声或有噪声）、在量子硬件（Google、IonQ、AQT、Pasqal）上运行、电路优化和编译、噪声建模和表征，或量子实验和基准测试（VQE、QAOA、QPE、随机基准测试）时使用此技能。
---
...
circuit = cirq.Circuit(
    ...
    cirq.CNOT(q0, q1),       # 以 q0 为控制，q1 为目标的 CNOT 门
...
### 量子实验
有关设计实验、参数扫描、数据收集和使用 ReCirq 框架的信息，请参见：
